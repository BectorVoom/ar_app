...
**Document Status:** Updated with Stabilization Identity Hardening, Jitter-Tolerant Hit Stability, and Explicit Preview Error Surfacing  
**Language:** English  
...
The product requirement is to start recording from the AR route, keep recording lifecycle tied to that route, capture microphone audio, and allow the user to explicitly download/export the validated result.
...
- after a successful validated finish, the app retains `lastCompletedRecording` in session state until AR route exit or a newer validated recording replaces it,
...
- retain the last validated recording as `RecordedVideoArtifact`
...
- the app must retain the finalized source URI in `lastCompletedRecording` during the current AR-route session.
